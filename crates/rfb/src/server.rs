...
use crate::rfb::{
    ...
    ProtoVersion, ProtocolError, ReadMessage, SecurityResult, SecurityType,
...
pub enum HandshakeError {
    ...
    IncompatibleVersions { client: ProtoVersion, server: ProtoVersion },

    #[error(
        "incompatible security types (client choice = {choice:?}, server offered = {offer:?})"
    )]
    IncompatibleSecurityTypes { choice: SecurityType, offer: SecurityTypes },
            ...
            match req {
                Ok(client_msg) => match client_msg {
                    ...
                    ClientMessage::FramebufferUpdateRequest(f) => {
                        ...
                        let mut fbu =
                            self.server.get_framebuffer_update().await;
